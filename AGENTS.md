...
- Embeddings/cache: `EMBEDDING_MODEL` (default `all-MiniLM-L6-v2`), semantic cache habilitado por defecto con TTL (`CACHE_TTL_SECONDS`), y subset de schema dinámico limitado por `SCHEMA_MAX_TABLES` para reducir tokens. Si hay Redis, puedes habilitar `USE_REDIS_CACHE=true` para cache distribuido (embeddings/resultados). Usa `DISABLE_HISTORY=true`/`USE_REDIS_CACHE=false` en entornos con datos sensibles y aplica limpieza de caches/historial según políticas.
