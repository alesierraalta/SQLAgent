...
- Flags adicionales para `query`:
  ...
  - `--simple`: Modo simple (oculta banners y decoración, solo datos)
  - `--no-analysis`: Oculta el bloque de análisis/razonamiento
  - `--show-sql`: Muestra el SQL generado incluso sin modo verbose

## 2) Configuración y Mantenimiento
- Listar configuración actual:
  `python -m src.cli config list`
- Obtener valor de una clave:
  `python -m src.cli config get QUERY_TIMEOUT`
- Modificar una configuración (persiste en .env o config local):
  `python -m src.cli config set QUERY_TIMEOUT 60`
  `python -m src.cli config set USE_FAST_MODEL false`
...
## 5) Ejemplos avanzados (según base de datos de ventas genérica)
...
## 7) Notas de seguridad/validación
...
## 9) Cache con Redis (Docker) para menor latencia
...
## 11) Chat CLI (modo conversación)
- Lanzar: `python -m src.cli_chat --mode safe`.
- Comandos clave: `/config` (editar mode/format/limit/timeout interactivo), `/set key=val` (cambios rápidos), `/schema`, `/settings` (ver config), `/history`, `/retry`, `/sql`.
